...
$totalMensaje = $consulta9["0"]["ciu"];

$consulta4 = $this->db->select("*")->from("mensaje")->get();
?>
...
<head>
    ...
    <title>Mensajes de personas interesadas</title>
...
?>
  ...
  <header class="header bg-gray-900 text-white">
    <nav class="flex flex-wrap items-center justify-between py-4 px-8">
        ...
        <div class="flex items-center space-x-4">
            <a href="<?=base_url()?>Accesoad/principal/<?=$this->session->id_usu?>/<?=$this->session->token?>" class="nav-link"><i class="fas fa-reply"></i>  Regresar</a>
        </div>
    </nav>
</header>
...
?>
        <div class="bg-white rounded-lg shadow p-6 mb-8">
            ...
            <h2 class="text-xl font-semibold text-gray-800 mb-4"><?=$mensaje["nombre"]?></h2>
            <button class="text-red-600 hover:text-red-800" onclick="showDeleteModal(<?=$mensaje["id"]?>)">
            <i class="fas fa-trash"></i>
        </button>
    </div>
            <p class="text-gray-500"><span class="font-semibold"><i class="fas fa-envelope mr-2"></i>Correo:</span> <a href="mailto:<?=$mensaje["correo"]?>"><?=$mensaje["correo"]?></a></p>
            <p class="text-gray-500"><span class="font-semibold"><i class="fas fa-phone mr-2"></i>Teléfono:</span> <a href="tel:<?=$mensaje["tel"]?>"><?=$mensaje["tel"]?></a></p>
            <p><span class="font-semibold"><i class="fas fa-envelope mr-2"></i>Mensaje:</span> <?=$mensaje["mensaje"]?></p>
            <p class="text-sm text-gray-500"><i class="fas fa-calendar mr-1"></i>Fecha: <?=fecha2($mensaje["fecha"]);?></p>
        </div>
   <?php 
  endforeach;
endif;
  ...
  <?php 
if ($totalMensaje == 0 || $totalMensaje == 1 || $totalMensaje == 2) {
  $s=1;
}
else{
  $s=2;
...
<div id="deleteModal" class="fixed inset-0 flex items-center justify-center z-10 hidden">
  <div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <p class="mb-4">¿Realmente deseas eliminar este mensaje?</p>
...
<script>
  ...
  function deleteResena() {
    ...
    $.ajax({
            "url" : appData.uri_ws+"backend/deletemensaje/",
            "dataType" : "json",
            "type"  :   "post",
            "data"  :   {
                "id" : idresenia
