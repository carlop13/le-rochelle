...
<head>
    ...
    <script>
        ...
        function cargarMasPropiedades(contador, numero) {
            $.ajax({
                url: appData.uri_ws + "backend/obtenerPropiedadesrent", // Endpoint original intacto
                method: "POST",
    ...
    <style type="text/css">
        ...
        .main-content {
            ...
            max-width: 1200px;
            margin: 0 auto;
            /* Padding ajustado a 20px en la parte superior */
        ...
        .modern-badge {
            ...
            top: 15px;
            left: 15px;
            background: #de4547; /* Rojo para resaltar Alquiler */
            color: white;
            padding: 5px 12px;
            border-radius: 20px;
            font-size: 12px;
...
<body class="page-propiedades">
    ...
    <main class="main-content">
        ...
        <?php 
        $contpro = $this->db->select("count(id) as id")->from("propiedad")->where("disponible",1)->where("estado","Alquiler")->get()->row()->id;
        if ($contpro > 0) :
        ?>
            <h2 class="titulo-seccion">Propiedades en Renta</h2>
        <?php else: ?>
            <h2 class="titulo-seccion">Por el momento no hay propiedades en Renta</h2>
        <?php endif; ?>
        ...
        <?php if ($contpro > 6) : ?>
            <div class="btn-ver-mas-container">
                <button class="btn-ver-mas" value="0" onclick="cargarMasPropiedades(this.getAttribute('data-contador'),<?=$numero?>)" id="botonCargarMas" data-contador="<?= $contador ?>">Ver Más Propiedades en Renta</button>
            </div>
        <?php endif;?>
