...
?>
  <header class="header bg-gray-900 text-white">
    <nav class="flex flex-wrap items-center justify-between py-4 px-8">
        ...
        <div class="flex items-center space-x-4">
            <a href="<?=base_url()?>Accesoad/solicitud/<?=$this->session->id_usu?>/<?=$this->session->token?>" class="nav-link"><i class="fas fa-reply"></i>  Regresar</a>
        </div>
    </nav>
</header>
  ...
  <div class="container mx-auto px-4 content">
    ...
    <?php 
    $cons = $this->db->select("id_propi")->from("propiedad")->where("id",$id_info)->get()->row();
    $id_propied = $cons->id_propi;

    $consuk = $this->db->select("nombre,ap")->from("propietario")->where("id",$id_propied)->get()->row();
    $nom = $consuk->nombre;
    $ap = $consuk->ap;

if ($nom != "") :
  ?>
      <h1 class="text-3xl font-semibold text-gray-800 mt-6 mb-6">Solicitudes de <?= $nom ?> <?= $ap ?></h1>

  <?php 
else:
  ?>
<h1 class="text-3xl font-semibold text-gray-800 mt-6 mb-6">Solicitudes</h1>
  ...
  ?>
      ...
      <?php if($totalSolitud==0): ?>
        ...
        No tienes solicitudes por esta propiedad aún
      </h2>
        <?php else: ?>
...
<button class="bg-red-500 text-white px-4 py-2 rounded ml-2 btn-elim" onclick="openModal('<?= $solicitud["id"] ?>','<?= $nombre_cliente ?>')">
